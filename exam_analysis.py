...
[STEP 0 — 가장 먼저: 시험지를 한 번 훑어 객관식/서답형 수를 시각적으로 카운트]
한국 고등 영어 시험지의 **서답형 문항은 거의 항상 다음 마커가 명시적으로 적혀 있습니다**:
  · "[서답형]" / "(서답형)" 머리말
  · "[서술형]" / "(서술형)"
  · "서답형 1번", "서술형 1번", "서답형-1" 같은 번호
  · 시험지 후반부 별도 섹션에 "서답형" 또는 "서술형" 헤더로 묶여 있음

이 마커가 보이는 문항은 **무조건** is_subjective = true 입니다. choices = [] 로.

[is_subjective — 시각 단서 우선순위]
1순위 (확정): 위 STEP 0 의 "서답형/서술형" 마커가 머리말에 적혀 있다 → true
2순위: ①②③④⑤ 선지가 있고 번호를 고르는 형태 → false (choices 채움)
3순위: 1·2번에 해당 안 되면 답란/빈칸·괄호·번역란 유무로 판단
       · 답을 적는 공간이 있다 → true
       · 유형이 "서술형", "단답형", "조건영작", "영작", "요약문 작성" → true

[교차 확인 — 자체 검증]
- 한국 고등 내신은 보통 객관식 17~22 + 서답형 3~8 구성.
  모두 객관식이거나 모두 서답형이면 거의 확실히 분류 오류 → 처음부터 다시 보세요.
- 시험지 표지/안내문에 "객관식 N문항 / 서답형 M문항" 비율이 명시돼 있으면 그 숫자에 맞추세요.
- choices 가 비어 있는데 is_subjective=false 면 모순. 반드시 일치시키세요.
- 마지막에 출력 직전 questions 배열의 is_subjective 비율을 확인해 위 비율 가이드와 어긋나면
  다시 분류하세요.
...
def gen_killer_deep(api_key: str, meta: ExamMeta, qs: list[Question],
                    ...
                    model: str = TEXT_MODEL_DEFAULT) -> list[KillerDeep]:
...
def gen_blog_body(api_key: str, meta: ExamMeta, qs: list[Question],
                  ...
                  seed: int | None = None,
                  model: str = TEXT_MODEL_DEFAULT) -> dict:
    if not qs:
    ...
    obj_n = sum(1 for q in qs if not q.is_subjective)
    sub_n = sum(1 for q in qs if q.is_subjective)
    fmt_lines = (f"- 객관식: {obj_n}문항\n- 서답형: {sub_n}문항")
...
- 차트 캡션 5종 각 1~3 문장. {('범위 데이터가 없으므로 scope 캡션은 빈 문자열' if not scope_dist else '')}
...
{{
  ...
  "seung": "...",
  "gyeol": "...",
  "captions": {{"type":"...","format":"...","scope":"...","difficulty":"...","location":"..."}}
}}
"""
    ...
    return {
        ...
        "captions": {
            ...
            "format": humanize_text(captions.get("format", "")),
...
def chart_format_distribution(qs: list[Question], theme: ThemeName = "editorial") -> bytes:
    """객관식 vs 서답형 비중 — 원그래프."""
    pal = chart_palette(theme)
    if not qs:
        return b""
    obj = sum(1 for q in qs if not q.is_subjective)
    sub = sum(1 for q in qs if q.is_subjective)
    if obj + sub == 0:
        return b""
    sizes = [obj, sub]
    labels = [f"객관식\n{obj}문항", f"서답형\n{sub}문항"]
    color_obj = pal["accents"][0]
    color_sub = pal["accents"][3] if len(pal["accents"]) > 3 else pal["accents"][1]

    fig, ax = plt.subplots(figsize=(7, 5.5), facecolor=pal["paper"])
    wedges, texts, autotexts = ax.pie(
        sizes, labels=labels, colors=[color_obj, color_sub],
        autopct="%1.1f%%", startangle=90,
        pctdistance=0.72, labeldistance=1.12,
        textprops={"fontsize": 12, "color": pal["ink"], "fontweight": "bold"},
        wedgeprops={"edgecolor": pal["paper"], "linewidth": 3},
    )
    for autotext in autotexts:
        autotext.set_color("#FFFFFF")
        autotext.set_fontsize(13)
        autotext.set_fontweight("bold")
    ax.set_title("객관식 vs 서답형 비중", loc="left", fontsize=14,
...
def make_charts(qs: list[Question], theme: ThemeName) -> dict[str, bytes]:
    return {
        ...
        "format":     chart_format_distribution(qs, theme),
...
def _generate_outputs(api_key: str, meta: ExamMeta, qs: list[Question],
                      ...
                      original_images: list[bytes],
                      model: str = TEXT_MODEL_DEFAULT) -> tuple[ReportPayload, bytes, bytes, str]:
    """§3 단일 클릭으로 모든 산출물 생성."""
    kds = gen_killer_deep(api_key, meta, qs, original_images, model=model)
